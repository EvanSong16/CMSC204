...
public class NotationStackTest {
	...
	public void testPopStudent() {
		...
		assertEquals(large, doubleS.pop(), 0.001);
		assertEquals(medium, doubleS.pop(), 0.001);
		assertEquals(small, doubleS.pop(), 0.001);
	...
	public void testPushStudent() {
		...
		assertTrue("Adding a large positive number", doubleS.push(huge));
		assertEquals(4, doubleS.size());
		assertTrue("Adding a negative number", doubleS.push(negative));
		assertEquals(5, doubleS.size());
		Assertions.assertThrows(StackOverflowException.class, () -> {
			doubleS.push(limitBreaker);
		});
	}
	...
	public void testToStringStudent() {
		//Use the doubleQ for student tests
		assertEquals("0.232.4655.79", doubleS.toString());
		doubleS.push(huge);
		assertEquals("0.232.4655.792345.22", doubleS.toString());
		doubleS.pop();
		doubleS.pop();
		assertEquals("0.232.46", doubleS.toString());
	}
